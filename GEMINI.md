...
- **Bridge macOS 开发红线**:
  ...
  - **侧边栏折叠导航闭环**: 侧边栏收起状态下必须保持顶部标题下拉 Quick Switcher Menu (`ToolbarTitleMenu`) 与 `⌥1`~`⌥6` (Option+1~6) 全局快捷键双重响应机制；Toolbar 下拉菜单统一施加 `.menuIndicator(.hidden)` 规避系统箭头重复冲突；右侧 Toolbar 必须按“页面高频操作（搜索/排序/筛选）-> 分隔线 -> 系统全局配置（性能模式/断开连接）”三段式层级规范编排，切换模块不得破坏用户当前的侧边栏折叠与沉浸状态。
