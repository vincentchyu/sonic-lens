...
- **日期**: 2026-04-23
    - **特性摘要**: Bridge mini 播放条与三端播放入口改为以 `WS now_playing` 为播放存在性的最高优先级事实源；客户端静默检测只负责冻结进度与展示“已暂停/已停止更新”，不再把仍存在的播放对象退化成“无活动播放”
    - **链接**: [Bridge WS 优先播放存在性与 mini 播放条修复清单](memory/2026-04-23/bridge_ws_authoritative_now_playing_manifest.md)

## 2026-04-22

- **日期**: 2026-04-22
    - **特性摘要**: 公众号文章生成正式收敛为“当天独立文章目录 + `output/` 兼容符号链接入口”模式，Markdown/HTML/素材引用规则与“确认最终读者版后再同步草稿箱”的发布门槛一起固化为长期流程约束
    - **链接**: [公众号文章输出目录化闭环清单](memory/2026-04-22/wechat_article_output_closure_manifest.md)
